...
[Troubleshooting](../troubleshooting.md) for symptom-indexed fixes, or the
[glossary](../concepts/glossary.md) for a term you don't recognize.

## Does it run on Intel Macs?

No. macOS support requires Apple silicon and Apple Virtualization.framework;
there is no Intel Mac backend. See
[Host requirements](../concepts/backends.md#macos).
...
[Troubleshooting: KVM unavailable](../troubleshooting.md#microagent-doctor-reports-kvm-unavailable-on-linux).
...
See [`microagent perf`](../cli/perf.md) for the full command set, or
[Reference measurements](../cli/perf.md#reference-measurements) for iteration
...
images always pull anonymously. See [`microagent registry`](../cli/registry.md).
...
another host backend. See [Host requirements: WSL](../concepts/backends.md#wsl).

## Can an agent see my API keys?

[Credential swap](../concepts/egress-mediation.md#credential-swap) keeps the key
...
[semantic broker grant](../guides/broker-grants.md) when a bounded response must
...
[`microagent secret`](../cli/secret.md). The value is materialized inside the
...
persist it anywhere. See [Deliver secrets](../guides/secrets.md).

## Can two VMs share a volume?

No. A [named volume](../concepts/storage.md#named-volumes) is single-attach: at
...
are in [limitations](../concepts/limitations.md).
...
recreating it. See [Quickstart](quickstart.md) for the
one-shot path and [Persistent workspaces](../guides/persistent-workspaces.md) for
...
[Storage: the rootfs](../concepts/storage.md#the-rootfs).
...
[Egress mediation](../concepts/egress-mediation.md#the-egress-modes) and
`microagent egress <name>` to see what a workspace actually tried to reach.

## See also

- [Troubleshooting](../troubleshooting.md) - fixes indexed by the error you're seeing
- [Limitations](../concepts/limitations.md) - the deliberate refusals and where to go instead
- [Coming from Docker](coming-from-docker.md) - command-by-command mapping
