...
| `docker stop web` | `microagent halt web` | Graceful shutdown; `stop` also works as a pure alias of `halt`. If the VM doesn't shut down cleanly within the fixed graceful window, follow up with `microagent kill`. See [halt, not stop](#halt-not-stop). |
