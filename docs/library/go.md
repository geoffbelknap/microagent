...
`vmkit.NewRuntimeContract()` exposes high-level feature records and the typed
operation registry shared by the CLI and MCP adapters. Use
`vmkit.OperationContracts()` for stable operation identities, adapter aliases,
and operation-level capability requirements. Library code should resolve stable
IDs with `vmkit.OperationContractByID()`; adapter coverage can use
