...
| `pkg/modelservice` | `Options`, `Attachment`, `Attach`, `Release`, `UpstreamResolver`, `PairOptions`, `Pair`, `PendingRelease` |
...
An application embedding `pkg/workspace` on macOS must set
`MICROAGENT_EGRESS_DATAPATH_BIN` to an installed `microagent` CLI before it
starts a mediated Apple VF workspace. The library does not assume the
embedding process implements microagent's hidden datapath command. Startup
returns `Response.DatapathStartupFailure` when the selected executable exits
early or its CA material is unavailable.
...
| `Progress` | `operation.ProgressFunc` callback for typed workspace and rootfs phases; the library never renders terminal output |
...
Use `modelservice.Pair` to prepare a workspace's model before calling `Run`,
`Create`, or `Start`. It downloads missing models, acquires a runner hold,
attaches the host service, and sets the guest model URLs. Runner and mediation
settings use the same spec fields and environment defaults as the CLI.

```go
opts := workspace.DefaultOptions()
opts.Name = "research"
opts.ImageRef = "alpine"
opts.ExecCommand = "printenv OPENAI_BASE_URL"
opts.Model = "Qwen/Qwen2.5-0.5B-Instruct-GGUF/qwen2.5-0.5b-instruct-q4_k_m.gguf"

release, err := modelservice.Pair(ctx, &opts, modelservice.PairOptions{
    ExecPath: microagentPath,
})
if err != nil {
    return err
}
...
`Pair` leaves options unchanged on failure and releases resources acquired
before attachment or event persistence failed. Its returned cleanup function
releases the service and runner hold on a best-effort basis. With no model,
`Pair` returns a no-op cleanup function.

For detached workspaces, keep the pairing alive after `Start` returns. Before
halt, kill, or delete, call `modelservice.PendingRelease(stateDir, name, backend)`
to capture cleanup; invoke the returned function after the lifecycle operation
succeeds. Capturing it before deletion preserves the model reference. Pair again
when starting a stopped workspace.

For an already-managed runner, `modelservice.Attach` accepts a
`modelrunner.Record` and returns only the service attachment. Set `ModelTarget`
to its address and `ModelTargetStable` to true; the caller retains runner ownership.
Empty attachment `Mode` forwards bytes; `local-allow` and `policy` select HTTP
mediation. `policy` requires `PolicyURL` or `PolicyFile`. Call `modelservice.Release`
to release an attachment made through this lower-level API.
...
| `workspace.Resize` | Grow or shrink a stopped workspace's rootfs disk in place (`ResizeOptions.Progress` reports typed check, disk, filesystem, verification, and publication phases; refuses while running or while snapshots exist; returns `ResizeResult` with the new `vmkit.DiskUsage`) |
...
| `workspace.GetArtifact` / `workspace.GetArtifactWithOptions` | Copy a declared output artifact from a stopped workspace; the options form reports typed phase and byte progress |
| `workspace.Copy` / `workspace.CopyWithOptions` | Copy files between the host and a stopped workspace disk; the options form reports typed phase and byte progress |
...
| `workspace.Clone` / `workspace.CloneWithOptions` | Clone a stopped/prepared workspace; the options form reports typed copy progress against the source workspace's logical byte total |
...
| `workspace.GC` | Reconcile running workspace records as one bounded batch; returns `GCResult` with reaped targets and per-target `GCFailure` values instead of stopping at the first failure |
...
`CopyWithOptions`, `GetArtifactWithOptions`, and `CloneWithOptions` accept an
`operation.ProgressFunc`. The original functions are shorthand with no
callback. Progress messages describe operations rather than caller paths, and
completed transfers report trustworthy byte totals.
...
Set `workspace.Options.Progress` to receive typed connect, retry, execute, and
first-output boundaries. The first-output event is emitted before its chunk is
delivered, so a presentation adapter can stop before writing stream data.
...
guest sends a reference and the broker injects the credential into the
upstream request. Response-side guarantees depend on the endpoint's explicit
assurance. Declare it per workspace with `Options.Broker`
...
the host-side `Secret` reference, and guest wiring (`GuestListen`,
`BaseURLEnv`, `Proxy`). It also carries the `Capture` opt-in, optional
`UpstreamCAFile`, `Assurance`, optional semantic `Grant`, and proxy
`ConnectAllowlist`. Request fills transport defaults such as the vsock port
and guest listen address.
`BrokerCredentialDisclosure` names the response disclosure mode;
`BrokerCredentialDisclosureDenyExact` is the required semantic value.
...
cfg, err := workspace.ParseBrokerConfig(
	...
	workspace.BrokerSecurityOptions{
		Assurance: "semantic",                // finite typed operations
		GrantPath: "./anthropic-grant.yaml",  // YAML/JSON operation contract
	},
)
if err != nil {
	panic(err)
...
init via `workspace.RegisterProgressKernelInstaller` (a
`workspace.ProgressKernelInstaller` func; the indirection exists because
`pkg/kernel` depends on `pkg/workspace`). The callback receives the same typed
kernel progress that an explicit `kernel.InstallOptions.Progress` callback
would receive. The older `RegisterKernelInstaller` remains available for
embedders that do not need progress.
...
`workspace.BuildVerification` uses the measured base SHA-256 as a fresh
private clone's initial identity after checking the clone's type and size. It
does not reread a multi-gigabyte disk before first boot; quiescent status
verification still hashes the private disk normally.
...
`imagecache.PruneWithOptions` accepts `PruneOptions.Progress` and reports one
bounded current/total counter while reconciling the index. `Prune` is the
source-compatible no-callback shorthand. The model store offers the same pair
as `model.PruneWithOptions` and `model.Prune`.
...
Set `diagnostics.Options.Progress` to receive typed probe boundaries. Fast
checks may replace one another without presentation; a slow current probe can
be shown without parsing diagnosis text.
...
| `microagent gc` | `workspace.GC` (bounded batch reconciliation with per-target failures) |
...
| `microagent commit` / `microagent image push` | `commit.Commit` / `commit.Push` (`commit.Options.Progress` spans local commit phases; push has its own progress callback) |
...
| `microagent volume` | `volume.CreateWithOptions` / `volume.ResizeWithOptions` for progress-aware mutations; `volume.Create` / `volume.Resize` are no-callback shorthands; `volume.List` / `volume.Get` / `volume.Remove` / `volume.Attach` cover the remaining lifecycle |
...
| `microagent perf` / `microagent perf boot` / `microagent perf ready` / `microagent perf footprint` / `microagent perf steady` | `perf.Boot` / `Ready` / `Footprint` / `Steady` |
