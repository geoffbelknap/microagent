...
[state and identity](../concepts/state-and-identity.md#linux-firecracker-socket-path-budget).
...
workspace. See [`profiles`](profiles.md) for the values. Use `--memory`,
...
The ref is stored with the workspace, so every [`start`](start.md) re-pairs
it - the host model server is re-ensured and `OPENAI_BASE_URL` points the guest
at it. See [`model`](model.md) for serving and release semantics.
...
| `--role <role>` | Caller-supplied role label. Defaults to `workload`. microagent records it in requests, state files, and events but does not interpret it - see [state and identity](../concepts/state-and-identity.md) |
...
- `--model <ref>` - pair the workspace with a locally served model; every
  `start` re-pairs it
- `--restart <policy>` - what [`supervise`](supervise.md) does when it exits
...
| `--restart <policy>` | `never`, `on-failure`, or `always`. Enforced by [`supervise`](supervise.md) |
...
| Flag | Description |
|---|---|
| `--secret NAME=<scheme>:<ref>` | Deliver a secret to `/run/secrets/NAME`, re-resolved each start. Repeatable. See [`secret`](secret.md) |
...
| Flag | Description |
|---|---|
| `--model <ref>` | Pair the workspace with a locally served HuggingFace GGUF model. See [`model`](model.md) |
...
existing workspace's [snapshot](snapshot.md) instead of building from an
...
- [`start`](start.md) - boot the workspace you created
- [`halt`](halt.md) - shut it down again (`stop` is an alias)
- [`delete`](delete.md) - remove it and its state
- [State and identity](../concepts/state-and-identity.md) - what the workspace record holds
- [Network modes](../concepts/networking.md) - `user`, `isolated`, and published ports
