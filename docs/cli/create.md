...
| `--egress <mode>` | [Egress mediation](/concepts/egress-mediation/) mode: `guarded` (default; deny the inside, allow public), `strict` (deny non-allowlisted), or `off`. Persisted with the workspace |
