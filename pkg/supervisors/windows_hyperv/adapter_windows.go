...
func (a defaultAdapter) PrepareNetwork(ctx context.Context, spec computeSystemSpec) (networkAttachment, error) {
	...
	case "user":
		// user-mode outbound is realized on Windows by the managed NAT HNS
		// network: the guest gets a NAT-backed endpoint with default-route
		// egress. There is no pasta/firecracker user-mode path on Windows.
	...
	default:
		return networkAttachment{}, fmt.Errorf("hyperv network.mode %q is unsupported; use user or isolated", network.Mode)
	}
}
...
func buildComputeSystemDocument(spec computeSystemSpec) ([]byte, error) {
	...
	// "user" is the only external attached mode; PrepareNetwork realizes it as
	// the managed NAT HNS network and rewrites the runtime mode to "nat", so the
	// runtime Create path arrives here with "nat". Accept both so the test path
	// (which calls this directly with "user") and the runtime path agree.
	if network := spec.Config.Network; network != nil && (network.Mode == "user" || network.Mode == "nat") {
