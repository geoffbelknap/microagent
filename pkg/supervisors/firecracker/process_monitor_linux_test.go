...
func TestInspectPreservesRunningStateWhenRuntimeLeaseIsHeld(t *testing.T) {
	dir := t.TempDir()
	opts := Options{Name: "agent-1", StateDir: dir}
	req := vmkit.Request{
		Command:  "run",
	...
	if resp.Event == nil || resp.Event.State != vmkit.StateRunning {
		t.Fatalf("inspectWorkspace = %+v, want running while runtime lease is held", resp.Event)
	}
...
func TestAcquireRuntimeLeaseIsExclusive(t *testing.T) {
	...
	if second, err := acquireRuntimeLease(opts); err == nil {
		_ = second.Close()
		t.Fatal("second runtime lease acquired while first was held")
	}
	if err := lease.Close(); err != nil {
		t.Fatal(err)
	}
	third, err := acquireRuntimeLease(opts)
	if err != nil {
		t.Fatalf("runtime lease after release: %v", err)
	}
	_ = third.Close()
}
