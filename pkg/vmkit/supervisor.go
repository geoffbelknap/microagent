...
// ResolveEgressDatapathBin returns the explicitly selected microagent binary.
// The microagent CLI registers itself at process entry. Library embedders must
// select a binary explicitly; falling back to os.Executable would point at the
// embedding application, which need not implement --egress-datapath.
func ResolveEgressDatapathBin() string {
	return strings.TrimSpace(os.Getenv(EgressDatapathBinEnv))
}
