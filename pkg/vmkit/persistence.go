...
func PersistenceContract() ContractPersistence {
	return ContractPersistence{
		...
		Artifacts: []PersistedArtifact{
			...
			artifact("runtime.containment", "<workspace>/containment/{result.json}", "state-dir", PersistenceEvidence, "pkg/workspace", "0700/0600",
				"the marker directory is created atomically before freeze; phase results replace atomically", "marker presence denies start, restore, resume, and ordinary custody deletion even when result.json is missing or malformed", "freeze, severance, capture, stop, custody phase order",
				"retained in durable custody; ordinary workspace and captured-snapshot deletion are denied", "containment lifecycle", "retry containment after a crash; never remove the marker to recover execution"),
			...
			artifact("runtime.datapath-startup", "<workspace>/datapath-startup.json", "state-dir", PersistenceOperational, "Apple VF backend supervisor", "0700/0600",
				"atomic owner-only replacement before VM boot", "typed boundary, executable, exit status, diagnostic path, and sanitized reason", "one status per start attempt",
				"replaced on the next mediated start and retained until workspace deletion", "workspace lifecycle", "retry startup after correcting the reported datapath failure"),
			...
			artifact("audit.constraints", "workspaces/<workspace>/constraint-history.json", "state-dir", PersistenceAudit, "pkg/workspace and internal/eventhistory", "0700/0600",
				"cross-process lock plus atomic replacement after each effective manifest, config-disk, or verification write", "every retained record carries an independently reconstructable manifest snapshot and artifact hashes; malformed history is rejected", "array order is serialized commit order",
				"bounded to the latest 1024 revisions", "workspace lifecycle", "preserve the readable history and report malformed content"),
			...
			artifact("logs.datapath", "<workspace>/datapath.log", "state-dir", PersistenceRecoverable, "Apple VF backend supervisor", "0700/0600",
				"sanitized stderr is drained while the datapath runs", "not an authoritative state record; startup failure fields name the diagnostic", "byte stream order",
				"bounded to 64 KiB and retained until workspace deletion", "workspace lifecycle", "no recovery; correct the selected executable or CA failure and restart"),
