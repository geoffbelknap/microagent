...
#
# Proves that with `egress_mode: strict` + an allowlist, guest TCP is captured by
# the in-netns nftables REDIRECT, sniffed (HTTP Host / TLS SNI), and either
# forwarded to an allowlisted destination or denied fail-closed — and that the
# mediator process is torn down with the workspace (no orphan).
#
...
allow_ok = any(e.get("event") == "egress_allow" and e.get("host") == allow for e in events)
deny_ok = any(e.get("event") == "egress_deny" and e.get("host") == deny for e in events)
if not allow_ok:
...
print("egress mediation: allow ok, deny blocked, audit recorded, teardown clean")
PY
