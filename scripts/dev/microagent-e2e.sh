...
SCENARIOS=(
  ...
  "windows-hyperv-egress-host:scripts/dev/microagent-e2e-windows-hyperv-egress-host.sh:windows:vm"
...
SCENARIO_COVERAGE=(
  ...
  "windows-hyperv-egress-host|backend-specific|windows-hyperv|Egress mediation end to end: mediated user workspace on the no-uplink Private HNS network; allowlisted host MITM'd (our CA, egress_allow mitm:true), non-allowlisted host denied, DNS over TCP/53 through the mediator, and host-enforced no-bypass after guest egress-flush"
