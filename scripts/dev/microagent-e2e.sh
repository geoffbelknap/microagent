...
SCENARIOS=(
  ...
  "windows-hyperv-snapshot-host:scripts/dev/microagent-e2e-windows-hyperv-snapshot-host.sh:windows:vm"
...
SCENARIO_COVERAGE=(
  ...
  "windows-hyperv-snapshot-host|backend-specific|windows-hyperv|HCS save-state snapshot create/list/rm, start --from-snapshot restore, create --from-snapshot fork; tmpfs marker round-trip proves memory restore"
...
E2E_MATRIX=(
  ...
  "snapshot/pause/resume|backend-specific|firecracker,windows-hyperv|firecracker-lifecycle-host,lifecycle-deep,windows-hyperv-snapshot-host|vCPU pause/resume and memory snapshot/restore/fork are implemented on Firecracker and windows-hyperv (HCS pause/resume + HcsSaveComputeSystem save-state + RestoreState); memory snapshot is planned on apple-vf"
