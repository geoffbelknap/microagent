...
SCENARIO_COVERAGE=(
  ...
  "cred-swap-oauth2|backend-specific|linux-kvm|a hand-authored oauth2-cc swap entry (--egress-swap-config; no built-in provider shorthand exists for this strategy) boots under mitm egress, proving the mediator loads and validates an oauth2-cc entry and persists its reference-only fields; the credential-acquisition and injection data path itself is proven hermetically in internal/egress (TestE2E_OAuth2CC_*)"
