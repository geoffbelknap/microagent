...
func mcpTools() []map[string]any {
	return []map[string]any{
		...
		mcpTool("workspace.create", "Create or dry-run a workspace, including snapshot forks.", []string{"name"}, map[string]any{
			...
			"ttl":                         map[string]any{"type": "integer", "description": "Idle TTL in seconds; the VM is reaped after this long with no exec/connect (activity renews). Defaults to 7 days; 0 = permanent"},
