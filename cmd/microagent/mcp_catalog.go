...
func mcpTools() []map[string]any {
	return []map[string]any{
		...
		mcpTool("workspace.create", "Create or dry-run a workspace, including snapshot forks.", []string{"name"}, map[string]any{
			...
			"egress_swap_config":          map[string]any{"type": "string", "description": "Credential-swap config path; request injection is host-side, while upstream response behavior remains service trust"},
			"egress_max_bps":              map[string]any{"type": "integer", "description": "Per-flow mediated egress rate in bytes/sec; defaults to 100 MiB/s under broker/mitm, 0 = unlimited"},
			"egress_max_total_bytes":      map[string]any{"type": "integer", "description": "Cumulative mediated egress bytes before the breaching flow is torn down; defaults to 50 GiB under broker/mitm, 0 = unlimited"},
			"egress_max_conns":            map[string]any{"type": "integer", "description": "Concurrently mediated TCP connections; defaults to 256 under broker/mitm, 0 = unlimited"},
			"ttl":                         map[string]any{"type": "integer", "description": "Lifetime lease in seconds from VM start; activity does not renew it. Defaults to 7 days; 0 = permanent"},
			"cred_swap":                   map[string]any{"type": "array", "items": map[string]any{"type": "string"}, "description": "Inject a built-in provider's API key host-side: PROVIDER[=env:NAME|file:PATH|vault:PATH] (e.g. anthropic, openai). Request-side reference only, never a literal"},
			"broker_upstream":             map[string]any{"type": "string", "description": "Egress broker upstream base URL; request credentials are injected host-side. Response guarantees depend on broker_assurance"},
			...
			"broker_assurance":            map[string]any{"type": "string", "enum": []string{"semantic", "trusted-upstream"}, "description": "Required response-side trust contract. semantic requires broker_grant; trusted-upstream explicitly accepts the broad response relay"},
			"broker_grant":                map[string]any{"type": "string", "description": "Host YAML/JSON semantic grant path"},
			"brokers":                     map[string]any{"type": "array", "items": map[string]any{"type": "string"}, "description": "Declare multiple broker endpoints instead of single broker_* fields; each item is ;-separated key=value pairs including required assurance=<mode> and grant=<path> for semantic"},
		...
		mcpTool("workspace.events", "Read the joined workspace lifecycle, constraint, egress, broker, and secret-access trajectory. Defaults to a compact recent-event summary; pass format=full for complete source records.", []string{"name"}, map[string]any{"name": map[string]any{"type": "string"}, "state_dir": map[string]any{"type": "string"}, "format": map[string]any{"type": "string", "enum": []string{"summary", "full"}}, "limit": map[string]any{"type": "integer"}, "after_index": map[string]any{"type": "integer"}}),
	...
	return map[string]any{
		...
		"agent_experience": map[string]any{
			"defaults": []string{
				...
				"use the preview confirmation_token for workspace.kill, workspace.quarantine, and host-mutating install/setup/build operations",
